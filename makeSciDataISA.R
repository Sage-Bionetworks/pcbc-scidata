...
parallelSynGet <- function (x) {
  synapseLogin()
  synGet(x, downloadFile=FALSE)@fileHandle$fileName
}

fileNames <- foreach(x=isa$`Data Record Accession`, .packages=c("synapseClient")) %dopar% parallelSynGet(x)
# fileNames <- lapply(head(isa$`Data Record Accession`), 
#                     function(x) synGet(x, downloadFile=FALSE)@fileHandle$fileName)
isa$`Raw Data File` <- unlist(fileNames)

write.csv(isa, file='isa.csv', row.names=FALSE)
dC4Used %>% 
